...
final class FluidSTTService: STTService, @unchecked Sendable {
    ...
    func initialize(modelVersion: AsrModelVersion = .v3) async throws {
        let manager = try await CtcZhCnManager.load(
            useInt8Encoder: false,
            progressHandler: nil
        )
        self.ctcZhCnManager = manager
        // self.vadManager = try await VadManager()
    }

    func transcribe(audioURL: URL) async throws -> TranscriptionResult {
        guard let ctcZhCnManager else {
        ...
        let result = try await ctcZhCnManager.transcribe(audioURL: audioURL)
        ...
        logger.notice("Transcription done: duration=\(totalDuration)s")
        logger.debug("Transcription text: '\(result)'")

        return TranscriptionResult(text: result, duration: totalDuration, words: [], segments: [])
    }
}

// Replicates WordTimingMerger.mergeTokensIntoWords from FluidAudioCLI (not exported by the core library).
// Tokens use leading spaces as word boundaries (SentencePiece-style, normalised by CtcZhCnManager).
